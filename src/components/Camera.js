...
export default class OCRScreen extends React.Component {
  ...
  isInRectOfInterest = bounds => {
    ...
    if (x > xLeft && x < xRight) {
      ...
      const yDown =
        yCenter + RECT_HEIGHT / 2 + ERROR_MARGIN - this.state.camTop;
  ...
  renderTextBlocks = () => {
    return (
      <View style={styles.responseContainer} pointerEvents="none">
        <Text style={styles.response}>{this.state.reference}</Text>
      </View>
    );
  };
